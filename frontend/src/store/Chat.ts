		...
		if (!value) {
			...
			Api.get('/chat/invitations_channels')
				.then(({ data }) => {
					...
					console.log(
						'Obtenemos todos los canales en los que me han invitado',
			...
			ChatSocket.on('priv_msg:created', (createdMsg) => {
				...
				if (get(receptor)) {
					if (
						get(receptor).id === createdMsg.sender.id ||
						get(receptor).id === createdMsg.receiver.id
					)
			...
			ChatSocket.on('channelmember:created', (createdChannelMember) => {
				...
				if (get(currentUser).id === createdChannelMember.user_id) {
					Api.get('/chat/channel/' + createdChannelMember.channel_id).then((response) => {
					...
					joined_channels.update((channels) => {
						return channels.map((channel) => {
							if (channel.id === createdChannelMember.channel_id) {
								let index = get(userList).findIndex(
									(user) => user.id === createdChannelMember.user_id
								);
			...
			ChatSocket.on('channelmember:updated', (updatedMember) => {
				...
				if (get(currentUser).id === updatedMember.user_id) {
					joined_channels.update((channels) => {
						let chan_index = channels.findIndex(
							(channel) => channel.id === updatedMember.channel_id
						);
						if (chan_index !== -1) {
							let user_index = channels[chan_index].members.findIndex(
								(user) => user.id === updatedMember.user_id
							);
							if (user_index !== -1)
			...
			ChatSocket.on('channelmember:deleted', (deletedChannelMember) => {
					...
					let updatedChannels = channels.map((channel) => {
						if (channel.id === deletedChannelMember.channel_id) {
							channel.members = channel.members.filter(
								(user) => user.id !== deletedChannelMember.user_id
							);
						}
						return channel;
					});
				...
				if (get(currentUser).id === deletedChannelMember.user_id) {
					let index = get(joined_channels).findIndex(
						(channel) => channel.id === deletedChannelMember.channel_id
					);
					let findedChannel: Channel;
					if (!index) {
						index = get(invitations_channels).findIndex(
							(channel) => channel.id === deletedChannelMember.channel_id
						);
						findedChannel = get(invitations_channels)[index];
					} else findedChannel = get(joined_channels)[index];
					joined_channels.update((channels) => {
