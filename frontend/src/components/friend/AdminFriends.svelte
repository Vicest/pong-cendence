<script lang="ts">
	...
	$: {
		people = [...friend_list];
		let isFriend;
		if (currentPerson == undefined) isFriend == false;
		else isFriend = friend_list.some((friend: any) => friend.nickname === currentPerson.nickname);
		if (!isFriend) displayChat = false;
