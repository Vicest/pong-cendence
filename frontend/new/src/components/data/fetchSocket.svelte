    ...
    export async function fetchSocket() {
        return new Promise((resolve) => {
            ...
            new_socket.on('priv_message', (message) => {
                type_Channel.subscribe((value) => {aux_type_Channel = value;});
                // console.log("new priv msg",message, " - chat type ", aux_type_Channel)
                aux_user._privateMessages.push(message);
                if (aux_receptor == undefined || aux_type_Channel != "Friend")
                    return;
                if (aux_receptor.nickname == message.sender.nickname || aux_receptor.nickname == message.receiver.nickname)
            ...
            new_socket.on('group_message', (message) => {
                ...
                if (aux_receptor == undefined || aux_type_Channel != "Group")
                    return;
