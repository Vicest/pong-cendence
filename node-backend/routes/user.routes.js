...
userroutes.get("/refresh-session", (req, res) => {
  if (req.session.refresh) {
    ...
    res
      .status(500)
      .send({ error: "You must log with 42 auth for refreshing tokens" });
});
...
userroutes.get("/login", (req, res) => {
  res.redirect(
    `${process.env.INTRA_URL}/oauth/authorize?client_id=${process.env.CLIENT_ID}&response_type=code&redirect_uri=${process.env.REDIRECT_URI}`
  );
});
...
const getCoalition = async function (token) {
  ...
  const userCoalition = await axios
    .get(
      `${process.env.INTRA_URL}/v2/users/${userId}/coalitions`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
userroutes.get("/user/:login", getToken, async (req, res) => {
  const data = await fetch(
    `${process.env.INTRA_URL}/v2/users/${req.params.login}`,
    {
      method: "GET",
      headers: {
