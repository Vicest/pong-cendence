...
class LineSegment {
	...
	public reflect(incidencePoint: Point, source: Point): Point {
		...
		//--- TODO
		const reflectedPointAsMatrix: Matrix = reflMatrix.multiply(
			new Matrix(2, 1, [[reflectedPoint.at(0)], [reflectedPoint.at(1)]])
		);
	...
	*/
	...
	/*
	private solveQuadratic(origin: Point, direction: Vector): [number, number] {
		//TODO Cramer
		const xColumn: Vector = new Vector([direction.at(1), this.direction.at(1)]);
		const yColumn: Vector = new Vector([
			-direction.at(0),
			-this.direction.at(0)
		]);
		...
		let temporaryMatrixName: Matrix = new Matrix(2, 2, [
			xColumn.values,
			yColumn.values
		]); //FIXME really, change the name
		const divisor: number = temporaryMatrixName.determinant();

		if (divisor == 0) throw new Error('Solve Q for paralell lines.');
	...
	*/
	...
	private reflectPoint(point: Point): Point {
		...
		//--- TODO
		let refectedPointAsMatrix: Matrix = reflMatrix.multiply(
			new Matrix(2, 1, [[point.at(0)], [point.at(1)]])
		);
		return new Vector([
			refectedPointAsMatrix.at(0, 0),
			refectedPointAsMatrix.at(0, 1)
		]);
	}
}
...
class Paddle {
	...
	public move(distance: number): void {
		...
		if (moveDistance + this.moveRangePosition_ < 0)
			...
			moveDistance + this.moveRangePosition_ + this.line_.length >
			this.moveRangeMax_
		)
			moveDistance =
				this.moveRangeMax_ - this.line_.length - this.moveRangePosition_;
...
export class Match {
	constructor(
		...
		this.players = [
			...
			new Paddle(
				players[1],
				new Vector([this.width, startHeight]),
				this.paddleLength
			)
		];
		this.map = [
...
//		if (this.players[0].id == player) this.players[0].move(distance);
//		else if (this.players[1].id == player) this.players[1].move(distance);
//		else console.log('Ah, player interaction! *hides in a corner*');
//	}
//
...
//				const reflected: Point = wall.reflect(
//					lineIntersection.coords,
//					this.ball
//				);
...
//				const lineIntersection = paddle.intersection(
//					this.ball,
//					this.ballVector
//				);
//				if (lineIntersection.inSegment) {
//					const reflected: Point = paddle.reflect(
//						lineIntersection.coords,
//						this.ball
//					);
//					const displacementToCollision: number = lineIntersection.coords
...
//		if (Math.random() > 0.5) this.ballVector.values = [0.5, 1.5];
//		else this.ballVector.values = [-0.5, 1.5];
//	}
}
