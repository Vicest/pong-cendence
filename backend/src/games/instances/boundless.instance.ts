...
export class BoundlessInstance extends EventEmitter {
	public static readonly slug = 'boundless';
	...
	constructor(match: Match) {
		...
		this.state = {
			status: 'waiting',
			winnerId: -1,
			countdown: BoundlessInstance.waitingTime,
			players: [
				{
					...
					input: [],
					paddle: {
						width: BoundlessInstance.paddlesWidth,
						height: BoundlessInstance.paddlesHeight
					},
					ping: 0
				},
				{
	...
	private movePaddles() {
		this.players.forEach((matchPlayer, index) => {
			...
			if (this.state.players[index].y <= 0) {
				...
				BoundlessInstance.canvasHeight - BoundlessInstance.paddlesHeight
			) {
				isDown = false;
			}
			if (isUp) {
	...
	private moveBall() {
		...
		// Walls collision
		if (newY < 0) {
			newY = BoundlessInstance.canvasHeight - newY;
			//this.state.ball.speedY *= -1;
		} else if (newY > BoundlessInstance.canvasHeight) {
			newY = newY - BoundlessInstance.canvasHeight;
			//this.state.ball.speedY *= -1;
		}
	...
	private checkScore() {
		this.state.players.forEach((player, index) => {
			if (player.score >= BoundlessInstance.scoreToWin) {
