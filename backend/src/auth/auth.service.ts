...
export class AuthService {
	...
	public async grantTokenPair(data: User, twofastatus: boolean) {
		...
		if (!user) user = await this.usersService.create(data);
		const toSign:JwtUser = {login:user.nickname, id:user.id, isAdmin:user.isAdmin, twofaenabled:user.two_factor_auth_enabled, twofavalidated:twofastatus, iv:user.IV}
